...
官方模型页面：`https://www.modelscope.cn/models/FunAudioLLM/Fun-ASR-Nano-2512`。模型能力说明、远端文件清单和 revision 以该 ModelScope 官方仓库为准，程序不得通过抓取展示网页正文判断安装状态。

官方页面当前将 `Fun-ASR-Nano-2512` 标为 8 亿参数模型，模型表明确列出中文、英文、日文，并说明中文覆盖 7 种方言及多种地域口音。页面总述中的“31 个语种”同时覆盖了同系列 `Fun-ASR-MLT-Nano-2512`，不能据此把本方案使用的 Nano 模型误标为 31 语种；首版语言选项因此固定为 `auto`、`zh`、`en`、`ja`，其中 `auto` 必须经过真实模型测试后才能宣称可用。官方 README 声明支持低延迟实时听写，但示例接口仍以 `AutoModel.generate(...)` 为主，所以本项目的临时结果刷新频率、滚动窗口和 CPU 降频策略必须通过 Task 13 的真实音频测试确定，不能仅凭宣传文字推断。官方 TODO 仍列有时间戳和说话人区分，首版明确不提供这两项能力。

截至本设计复核时，ModelScope 仓库 API 返回 `master`，仓库总大小约 2.15 GB，顶层包含 `configuration.json`、`config.yaml`、`model.pt`、`multilingual.tiktoken`、`Qwen3-0.6B/` 和 `example/` 等内容。程序运行时必须读取对应 revision 的远端文件 API，不得把这里记录的大小或清单永久硬编码为未来版本事实。
...
检测器只检查明确目录和有限的标准子路径，不无界递归扫描整个磁盘。当前官方清单的必需内容包括 `configuration.json`、`config.yaml`、`model.pt` 以及 `Qwen3-0.6B` 下的配置与词表；检测器同时校验 revision 信息、远端清单中提供的大小或 SHA-256 和权重可读性。不能把官方当前清单中不存在的 `model.py` 当作固定必需文件；若未来 revision 增加远程代码文件，则按该 revision 的远端清单一并校验。文件检测通过后，在独立进程执行一次加载冒烟测试。
...
- 默认启用 ITN 和模型原生标点能力。
- 语言提供 `auto`、`zh`、`en`、`ja`。`auto` 通过省略 FunASR 的 `language` 参数使用模型原生检测，并必须用官方三语种样例真实验证；如果当前固定运行时不能可靠自动检测，设置页禁用 `auto` 并要求用户选择明确语种，不能静默回退为中文或伪造自动模式。
