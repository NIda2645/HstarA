...
async function fetchFake(url, options={}) {
  ...
  if(projectMatch){
    ...
    if(method === 'POST' && value.endsWith('/clone')){
      const body = JSON.parse(options.body);
      const source = projectRecords.get(body.source_project_id);
      if(!source) return response({ok:false, status:404, json:{detail:'Not Found'}});
      const project = {
        ...structuredClone(source),
        projectId,
        owner:structuredClone(body.owner),
        autosaveVersion:1,
      };
      projectRecords.set(projectId, project);
...
assert.equal(
  editorMessages(frameB).find(message => message.type === protocol.TYPES.OPEN_SESSION)?.payload?.entryMode,
  'workspace',
...
dispatchEditorMessage(frameB, protocol.createEnvelope({
  ...
  requestId:'reveal-b',
  context:contextB,
  payload:{project:projectRecords.get('project-b'), reason:'sources-synchronized'},
}));
await flushAsync();
assert.equal(frameB.hidden, false, 'empty editor should reveal after empty-source synchronization');
assert.equal(editorMessages(frameB).at(-1).type, protocol.TYPES.FIT_WORKSPACE);

assert.throws(() => host.openNodeSession({
  ...contextB,
  nodeId:'node-invalid-clone',
  projectId:'project-invalid-clone',
  cloneSourceProjectId:'project-b',
  cloneSourceCanvasType:'smart',
}, []), /clone source context is incomplete/);

const legacyCloneContext = {
  canvasType:'smart', canvasId:'canvas-legacy-clone', nodeId:'node-legacy-clone',
  projectId:'project-legacy-clone', projectName:'旧缓存副本', frameId:'frame-legacy-clone',
  cloneSourceProjectId:'project-b', documentWidth:1280, documentHeight:720,
};
elements.set(legacyCloneContext.frameId, createElement('iframe', legacyCloneContext.frameId));
host.openNodeSession(legacyCloneContext, []);
const legacyCloneFrame = frameFor(legacyCloneContext);
legacyCloneFrame.dispatch('load');
const legacyCloneSession = readyFrame(legacyCloneFrame, legacyCloneContext, 'ready-legacy-clone');
await flushAsync();
const legacyCloneRequest = fetchCalls.find(call => call.url === '/api/openshop/projects/project-legacy-clone/clone');
assert.ok(legacyCloneRequest, 'legacy clone metadata should still clone the source project');
assert.equal(
  Object.hasOwn(JSON.parse(legacyCloneRequest.options.body), 'source_owner'),
  false,
  'legacy clone metadata should omit an unavailable source owner',
);
dispatchEditorMessage(legacyCloneFrame, protocol.createEnvelope({
  type:protocol.TYPES.PROJECT_CHANGED,
  sessionId:legacyCloneSession,
  requestId:'reveal-legacy-clone',
  context:legacyCloneContext,
  payload:{project:projectRecords.get('project-legacy-clone'), reason:'sources-synchronized'},
}));
await flushAsync();
assert.equal(legacyCloneFrame.hidden, false);
assert.equal(await host.disposeProject(legacyCloneContext.projectId, legacyCloneContext), true);
