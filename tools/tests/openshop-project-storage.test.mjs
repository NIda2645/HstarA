...
async def api_lifecycle():
    with tempfile.TemporaryDirectory(prefix="hstara-openshop-api-") as app_root:
        ...
        async with httpx.AsyncClient(transport=transport, base_url="http://testserver") as client:
            ...
            legacy_source = copy.deepcopy(saved.json()["project"])
            legacy_source["projectId"] = "project-api-legacy"
            legacy_source["owner"] = {**owner, "nodeId": "node-api-legacy-source"}
            legacy_source_path = Path(main.OPENSHOP_DATA_DIR) / "projects" / "project-api-legacy.json"
            legacy_source_path.parent.mkdir(parents=True, exist_ok=True)
            legacy_source_path.write_text(
                json.dumps(legacy_source, ensure_ascii=False),
                encoding="utf-8",
            )
            legacy_clone_owner = {**owner, "nodeId": "node-api-legacy-clone"}
            legacy_cloned = await client.post(
                "/api/openshop/projects/project-api-legacy-clone/clone",
                json={
                    "source_project_id": "project-api-legacy",
                    "owner": legacy_clone_owner,
                },
            )
            assert legacy_cloned.status_code == 200, legacy_cloned.text
            assert legacy_cloned.json()["project"]["owner"] == legacy_clone_owner
            assert legacy_cloned.json()["project"]["previewAssetId"] == asset["assetId"]
