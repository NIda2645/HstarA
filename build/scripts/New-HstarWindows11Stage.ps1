...
function Get-TrackedFiles {
    ...
    $startInfo.Arguments = "-C `"$repoRoot`" -c core.quotepath=false ls-files -z --cached --others --exclude-standard -- `"$RelativeRoot`""
...
if ($AllowDirtyForTest) {
    $sourceGateArguments += '-AllowDirtyForTest'
}
Invoke-Native -Command 'powershell.exe' -Arguments $sourceGateArguments
Assert-CleanWorktree
