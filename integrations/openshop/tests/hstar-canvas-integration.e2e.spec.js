...
test('opens empty and sourced OpenShop nodes directly in the workspace', async ({page, request}) => {
  const runId = `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  const sourceImages = [
    {
  ...
  await openNode(page, frame, 'classic', emptyNode.id, 0, {welcome:'hidden'});
