...
SUPPORTED_PROVIDER_PROTOCOLS = {"openai", "apimart", "gemini", "linapi", "otuapi", "grsai", "toapis", "baofu", "bananarouter", "moonly", "volcengine", "runninghub", "jimeng", "lingjing", "codex", "gemini-cli"}
...
def is_qzz_base_url(base_url):
    return "img.688.qzz.io" in str(base_url or "").strip().lower()
...
class SmartImageTextRecognizeRequest(BaseModel):
    image_url: str = ""
    provider: str = "comfly"
    model: str = ""
    ms_model: str = ""
...
def is_apimart_provider(provider):
    ...
    return provider_protocol(provider) == "apimart" or "apimart.ai" in base_url or "img.688.qzz.io" in base_url

def is_qzz_image_provider(provider):
    base_url = str((provider or {}).get("base_url") or "").strip().lower()
    return is_qzz_base_url(base_url)
    ...
    async with httpx.AsyncClient(
        ...
        if image_refs:
            ...
            try:
                ...
                if not files:
                    raise HTTPException(status_code=400, detail="LinAPI 图生图需要可读取的参考图片文件")
...
def reference_to_data_url(ref, max_size=None):
    ...
    raw_url = str((ref or {}).get("url", "") or "").strip()
    path = output_file_from_url(raw_url)
    if not path:
        if raw_url.startswith(("blob:", "file://")):
            return ""
        if raw_url.startswith(("/output/", "/assets/")):
            return ""
        return raw_url
    if max_size:
        try:
...
def require_converted_image_refs(source_refs, converted_values, label):
    source_count = len([ref for ref in (source_refs or []) if isinstance(ref, dict) and ref.get("url")])
    values = [value for value in (converted_values or []) if value]
    if source_count and not values:
        raise HTTPException(status_code=400, detail=f"{label} 需要可用的参考图片输入，但当前图片无法转换为该平台支持的格式")
    return values
...
def friendly_image_error_detail(text, size="", model=""):
    ...
    visible_text = "\n".join(item for item in (text, message, code) if item)
    ...
    missing_reference_image = (
        ("请上传" in visible_text and ("参考图" in visible_text or "参考图片" in visible_text or "图片" in visible_text))
        or ("reference image" in lower_text and any(token in lower_text for token in ("upload", "missing", "required", "provide")))
        or ("input image" in lower_text and any(token in lower_text for token in ("upload", "missing", "required", "provide")))
    )
    if missing_reference_image:
        return "上游没有识别到参考图片。请确认当前节点图片已经作为参考图传入，并检查该平台是否要求公网图片 URL、专用 reference_image 字段或图片上传接口。"
    ...
    if any(token in lower_text for token in ("insufficient balance", "insufficient_user_quota", "quota", "balance")):
        return "insufficient balance / quota error: 余额或额度不足，请检查账户余额、预扣费或服务额度。"
...
async def generate_modelscope_provider_image(prompt, size, model, reference_images=None, provider=None):
    ...
    refs = require_converted_image_refs(source_refs, refs, "ModelScope 图生图")
    headers = {
...
async def generate_bananarouter_provider_image(prompt, size, model, reference_images=None, provider=None, quality=""):
    ...
    edit_url = provider_endpoint_url(provider, "image_edit_endpoint", "/v1/images/edits")
    refs = [ref for ref in (reference_images or []) if ref.get("url")]
    image_payloads = require_converted_image_refs(refs[:4], [reference_to_data_url(ref, max_size=1536) for ref in refs[:4]], "BananaRouter 图生图")
    quality = str(quality or "").strip().lower()
...
async def generate_moonly_provider_image(prompt, size, model, reference_images=None, provider=None, quality=""):
    ...
    for ref in source_refs:
        ...
        if url:
            refs.append(url)
    refs = require_converted_image_refs(source_refs, refs, "MoonlyAI 图生图")
...
async def generate_grsai_provider_image(prompt, size, model, reference_images=None, provider=None):
    ...
    source_refs = [ref for ref in (reference_images or [])[:14] if ref.get("url")]
    refs = require_converted_image_refs(source_refs, [reference_to_data_url(ref, max_size=1536) for ref in source_refs], "Grsai 图生图")
...
async def generate_gemini_provider_image(prompt, size, model, reference_images=None, provider=None):
    ...
    require_converted_image_refs(source_refs, image_parts, "Gemini 图生图")
...
async def generate_runninghub_entry_image(prompt, size, model, reference_images, provider, entry):
    ...
    async with httpx.AsyncClient(
        ...
        uploaded = require_converted_image_refs(source_refs, uploaded, "RunningHub 图生图")
...
async def generate_runninghub_provider_image(prompt, size, model, reference_images=None, provider=None):
    ...
    async with httpx.AsyncClient(
        ...
        image_urls = require_converted_image_refs(source_refs, image_urls, "RunningHub 图生图")
        if image_urls:
...
async def generate_ai_image(prompt, size, quality, model, reference_images=None, provider_id="comfly"):
    ...
    if is_qzz_image_provider(provider):
        return await generate_qzz_provider_image(prompt, size, model, reference_images, provider, quality=quality)
    ...
    async with httpx.AsyncClient(
        ...
        if image_request_mode == "openai-video-proxy":
            ...
            if image_refs:
                body["image_urls"] = require_converted_image_refs(
                    image_refs[:ONLINE_IMAGE_REFERENCE_MAX],
            ...
            try:
                ...
                if mask_refs:
                    ...
                    if mask_path:
                        ...
                        files.append(("mask", (os.path.basename(mask_path), fh, content_type_for_path(mask_path))))
                if not files:
                    raise HTTPException(status_code=400, detail="图生图需要可读取的参考图片文件")
                try:
...
"""
    ...
    if completed.returncode != 0:
        ...
        raise HTTPException(status_code=500, detail=detail)
    selected = (completed.stdout or "").strip().splitlines()
    return validate_external_executable(selected[-1].strip()) if selected else ""
...
def launch_external_app(executable: str, image_path: str) -> None:
    ...
    try:
        ...
        subprocess.Popen([executable, image_path], cwd=os.path.dirname(executable) or None, stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, close_fds=True, creationflags=flags)
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed to launch external application: {exc}") from exc

def media_bytes_from_url(url: str) -> tuple[bytes, str, str]:
    path = output_file_from_url(url)
    if not path:
        path = local_media_file_by_basename(filename_from_media_url(url, ""))
    if path and os.path.isfile(path):
        with open(path, "rb") as f:
            return f.read(), content_type_for_path(path), os.path.basename(path)
    text = str(url or "").strip()
    parsed = urllib.parse.urlparse(text)
    if parsed.scheme not in ("http", "https") or not parsed.netloc:
...
async def recognize_smart_image_text(payload: SmartImageTextRecognizeRequest):
    ...
    try:
        ...
        raw_items = parsed.get("texts") if isinstance(parsed, dict) else parsed
        if isinstance(raw_items, list):
            for index, item in enumerate(raw_items):
                value = item.get("text") if isinstance(item, dict) else item
                value = str(value or "").strip()
                if value:
                    items.append({"index": index, "text": value})
    except Exception:
        items = []
    if not items:
        for index, line in enumerate(re.split(r"[\r\n]+|[;；]", text)):
            ...
            if value:
                items.append({"index": index, "text": value})
    return {"success": True, "texts": items, "text": text, "model": resolved_model}
...
def protocol_from_payload(payload):
    ...
    if provider_id == "linapi" or "linapi.net" in base_url:
        return "linapi"
    if provider_id == "otuapi" or "otuapi.com" in base_url:
        return "otuapi"
...
async def test_provider_connection(payload: TestConnectionPayload):
    ...
    try:
        async with httpx.AsyncClient(timeout=15, trust_env=payload.use_system_proxy) as client:
            ...
            if resp.status_code >= 400:
                if is_qzz_base_url(base_url):
                    return baofu_default_model_payload(
                        status=resp.status_code,
                        message="QZZ 模型列表接口当前不可用，已使用兼容默认模型 gpt-image-2。",
                        raw={"models_error": resp.text[:300], "models_endpoint_model": "gpt-image-2"},
                    )
                if protocol == "volcengine":
            ...
            return {"ok": True, "protocol": protocol, "status": resp.status_code,
                "model_count": len(ids),
                "image_models": grouped["image"],
                "chat_models": grouped["chat"],
                "video_models": grouped["video"],
...
async def fetch_models_from_upstream(
    ...
    try:
        async with httpx.AsyncClient(timeout=30, trust_env=use_system_proxy) as client:
            ...
            if resp.status_code >= 400:
                if is_qzz_base_url(base_url):
                    payload = baofu_default_model_payload(
                        status=resp.status_code,
                        message="QZZ 模型列表接口当前不可用，已使用兼容默认模型 gpt-image-2。",
                        raw={"models_error": resp.text[:300], "models_endpoint_model": "gpt-image-2"},
                    )
                    return {
                        ...
                        "raw": payload["raw"],
                        "image_request_mode": normalize_image_request_mode(image_request_mode),
                    }
                if protocol == "volcengine":
                    detected, probe = await probe_volcengine_auto_detect(client, base_url, api_key)
                    if detected:
...
def save_and_reconcile_canvas_request(canvas_id, payload):
    with CANVAS_LOCK:
        canvas = load_canvas(canvas_id)
        ...
        if (
            ...
            and int(payload.base_updated_at) < current_updated_at
        ):
            if canvas_save_request_matches(canvas, payload):
                reconcile_openshop_canvas_projects(canvas)
                return canvas
        ...
        save_canvas(canvas)
        removed_openshop_projects = reconcile_openshop_canvas_projects(canvas)
        if (
...
async def update_canvas(canvas_id: str, payload: CanvasSaveRequest):
    canvas = await asyncio.to_thread(
        save_and_reconcile_canvas_request,
        canvas_id,
        payload,
    )
