...
MIGRATION_STATUSES = frozenset(
    {"preflight", "copying", "verifying", "switching", "completed", "cancelled", "failed"}
)
...
class MigrationManager:
    ...
    def _execute(self, task_id: str, *, resume: bool) -> None:
        ...
        for entry in manifest:
            ...
            if (
                ...
                and self._hash_file(destination) == entry["sha256"]
            ):
    ...
    def _commit_transaction(
        ...
        try:
            ...
            transaction.rmdir()
            previous = self.bootstrap.require()
            self.bootstrap.save(
                BootstrapConfig(
                    schema_version=1,
                    edition=previous.edition,
                    data_root=str(target),
                    last_started_version=previous.last_started_version,
