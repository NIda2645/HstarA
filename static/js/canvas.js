...
function normalizeMoonlyVipCombo(target, providerId, model){
    ...
    target.ratio = firstMoonlyRatioFor(model, resolution) || 'square';
    target.customRatio = '';
...
document.getElementById('imageMarkerLayer')?.addEventListener('pointerdown', beginImageMarkerPointer);
document.getElementById('imageMarkerLayer')?.addEventListener('pointermove', moveImageMarkerPointer);
document.getElementById('imageMarkerLayer')?.addEventListener('pointerup', endImageMarkerPointer);
document.getElementById('imageMarkerLayer')?.addEventListener('pointercancel', endImageMarkerPointer);
document.getElementById('imageMarkerLayer')?.addEventListener('pointerleave', endImageMarkerPointer);
...
function resizeEditDrawCanvas(){
    ...
    if(imageEditMode === 'marker') renderImageMarkers();
}
function setImageEditMode(mode, userTouched=false){
    ...
    imageEditMode = ['preview','marker','crop','outpaint','mask','brush','resize','grid'].includes(mode) ? mode : 'crop';
    ...
    if(isPreview){
        ...
        apply.style.display = imageEditMode === 'marker' ? 'none' : '';
        if(imageEditMode === 'marker'){
            title.textContent = '标记图片';
            sub.textContent = '点击图片添加标记，拖动标记可调整位置';
    ...
    else if(imageEditMode === 'marker') renderImageMarkers();
...
function openImageEditor(nodeId, initialMode='crop'){
    ...
    if(!['preview','marker','crop','outpaint','mask','brush','resize','grid'].includes(initialMode)) initialMode = 'crop';
...
function closeImageEditor(){
    ...
    const cropCanvasEl = document.getElementById('cropCanvas');
    cropCanvasEl.classList.remove('grid-custom-h', 'grid-custom-v', 'outpaint-mode', 'outpaint-warning', 'dragging-image', 'text-mode', 'marker-mode', 'resize-mode');
    imageMarkerDrag = null;
    const markerLayer = document.getElementById('imageMarkerLayer');
    const markerPanel = document.getElementById('imageMarkerPanel');
    if(markerLayer) markerLayer.innerHTML = '';
    if(markerPanel){ markerPanel.innerHTML = ''; markerPanel.classList.remove('active'); }
    cropCanvasEl.style.width = '';
    cropCanvasEl.style.height = '';
    const textCanvas = editTextCanvas();
    if(textCanvas){
...
function currentMarkerImage(){
    const node = nodes.find(n => n.id === cropState?.nodeId);
    if(!node?.url) return {node:null, image:null, display:null, markers:[]};
    if(!Array.isArray(node.markers)) node.markers = [];
    return {node, image:node, display:{url:node.url, name:node.name || 'image'}, markers:node.markers};
}
function markerLabel(marker){
    return String(marker?.objectName || marker?.label || marker?.name || '').trim();
}
...
function markerInputDisplayValue(marker){
    ...
    if(marker?.label) return normalizeMarkerObjectName(marker.label);
    if(marker?.status === 'identifying') return '识别中';
    if(marker?.status === 'failed') return normalizeMarkerObjectName(marker.error || '识别失败');
    return '';
}
...
function saveMarkerApiPreference(provider, model){
    try { localStorage.setItem('imageMarkerApiPreference', JSON.stringify({provider:provider || '', model:model || ''})); }
    catch(e){}
}
...
function markerModelOptions(providerId, selectedModel){
    const models = providerChatModels(resolveChatProviderId(providerId));
    const selected = selectedModel || preferredMarkerVisionModel(models);
...
function renderImageMarkers(){
    ...
    const {image} = currentMarkerImage();
    if(!image){ layer.innerHTML = ''; panel.innerHTML = ''; panel.classList.remove('active'); return; }
    const markers = image.markers || [];
    layer.innerHTML = markers.map((marker, index) => {
        marker.number = Number(marker.number || index + 1);
        return `<button class="image-marker-pin ${imageMarkerDrag?.markerId === marker.id ? 'dragging' : ''}" type="button" data-marker-id="${escapeAttr(marker.id || '')}" style="left:${Math.max(0, Math.min(1, Number(marker.x) || 0)) * 100}%;top:${Math.max(0, Math.min(1, Number(marker.y) || 0)) * 100}%" title="${escapeAttr(markerLabel(marker) || `#${marker.number}`)}">${IMAGE_MARKER_ICON_SVG}<span>${marker.number}</span></button>`;
    }).join('');
    ...
    panel.innerHTML = markers.length ? markers.map(marker => `<div class="image-marker-row ${escapeAttr(marker.status || '')}" data-marker-id="${escapeAttr(marker.id || '')}">
        ...
        </div>`).join('') : '<div class="image-marker-empty">点击图片添加标记点位</div>';
    panel.querySelectorAll('[data-marker-name]').forEach(input => {
        input.oninput = e => {
            const marker = currentMarkerImage().markers.find(item => item.id === e.target.dataset.markerName);
            if(!marker) return;
            marker.objectName = normalizeMarkerObjectName(e.target.value);
            marker.label = marker.objectName;
    ...
    panel.querySelectorAll('[data-marker-refresh]').forEach(btn => {
        btn.onclick = async e => {
            ...
            e.stopPropagation();
            btn.disabled = true;
            try{ await refreshImageMarkerById(btn.dataset.markerRefresh); }
...
function addImageMarkerAt(point){
    const {node, image, markers} = currentMarkerImage();
    if(!node || !image || !point) return null;
    const nextNumber = markers.reduce((max, marker) => Math.max(max, Number(marker.number) || 0), 0) + 1;
    const marker = {
        id:uid('marker'),
        number:nextNumber,
        x:point.x,
        ...
        label:'',
        status:'identifying',
        thumbnail:markerThumbnailAt(point.x, point.y),
        provider:image.markerProvider || markerProviderValue(),
        model:image.markerModel || markerModelValue(),
        createdAt:Date.now()
    };
    markers.push(marker);
    renderImageMarkers();
...
function deleteImageMarker(markerId){
    const {markers} = currentMarkerImage();
    const index = markers.findIndex(marker => marker.id === markerId);
    if(index < 0) return;
    markers.splice(index, 1);
    markers.forEach((marker, i) => { marker.number = i + 1; });
    renderImageMarkers();
    scheduleSave();
}
...
function markerProviderValue(){
    const select = document.getElementById('markerProviderSelect');
    return select?.value || currentMarkerImage().image?.markerProvider || resolveChatProviderId(settings.provider_id || '');
}
function markerModelValue(providerId=markerProviderValue()){
    const select = document.getElementById('markerModelSelect');
    return select?.value || currentMarkerImage().image?.markerModel || resolveChatModel('', providerId);
}
function refreshMarkerProviderControls(){
    ...
    if(!image || !providerSelect || !modelSelect) return;
    const pref = markerApiPreference();
    if(pref.provider && (!image.markerProvider || markerProviderNeedsUpgrade(image.markerProvider))){
        image.markerProvider = resolveChatProviderId(pref.provider);
        const prefModels = providerChatModels(image.markerProvider);
        if(pref.model && prefModels.includes(pref.model)) image.markerModel = pref.model;
    }
    providerSelect.innerHTML = chatProviderOptions(image.markerProvider || settings.provider_id || '');
    providerSelect.value = resolveChatProviderId(image.markerProvider || providerSelect.value || settings.provider_id || '');
    image.markerProvider = providerSelect.value;
    const models = providerChatModels(image.markerProvider);
    if(!models.includes(image.markerModel)) image.markerModel = preferredMarkerVisionModel(models);
    modelSelect.innerHTML = markerModelOptions(image.markerProvider, image.markerModel || settings.model || '');
    modelSelect.value = image.markerModel || '';
}
function bindMarkerProviderControls(){
    ...
    if(providerSelect){
        ...
        providerSelect.onchange = e => {
            ...
            const {image} = currentMarkerImage();
            if(!image) return;
            image.markerProvider = e.target.value;
            image.markerModel = preferredMarkerVisionModel(providerChatModels(image.markerProvider));
            saveMarkerApiPreference(image.markerProvider, image.markerModel);
            refreshMarkerProviderControls();
            scheduleSave();
        };
    }
    if(modelSelect){
        ...
        modelSelect.onchange = e => {
            ...
            const {image} = currentMarkerImage();
            if(!image) return;
            image.markerModel = e.target.value;
            saveMarkerApiPreference(image.markerProvider, image.markerModel);
            scheduleSave();
        };
    }
...
async function identifyImageMarker(marker){
    const {image, display} = currentMarkerImage();
    if(!image || !marker || !display?.url) return;
    const markerId = marker.id;
    const liveMarker = () => currentMarkerImage().markers.find(item => item.id === markerId);
    const target = liveMarker() || marker;
    target.status = 'identifying';
    target.provider = image.markerProvider || markerProviderValue();
    target.model = image.markerModel || markerModelValue(target.provider);
    target.error = '';
    target.thumbnail = markerThumbnailAt(target.x, target.y) || cleanDataImageUrl(target.thumbnail || marker.thumbnail || '');
    renderImageMarkers();
    try {
        const provider = target.provider;
        const data = await requestImageMarkerIdentify({
...
async function refreshSingleImageMarker(marker){
    if(!marker) return;
    const {image} = currentMarkerImage();
    marker.provider = image?.markerProvider || marker.provider || '';
    marker.model = image?.markerModel || marker.model || '';
...
async function refreshImageMarkerById(markerId){
    const marker = currentMarkerImage().markers.find(item => item.id === markerId);
    if(marker) await refreshSingleImageMarker(marker);
}
async function refreshAllImageMarkers(){
    const markers = currentMarkerImage().markers.slice();
    if(!markers.length) return;
    const btn = document.getElementById('markerRefreshBtn');
    if(btn) btn.disabled = true;
    try{
        for(const marker of markers) await refreshSingleImageMarker(marker);
    } finally {
...
function beginImageMarkerPointer(event){
    if(imageEditMode !== 'marker' || event.button !== 0) return;
    ...
    const {markers} = currentMarkerImage();
    if(pin){
        const marker = markers.find(item => item.id === pin.dataset.markerId);
        if(!marker) return;
        imageMarkerDrag = {markerId:marker.id};
        pin.setPointerCapture?.(event.pointerId);
        return;
    }
    if(Date.now() - imageMarkerLastAddAt < 160) return;
    imageMarkerLastAddAt = Date.now();
    addImageMarkerAt(point);
}
function moveImageMarkerPointer(event){
    if(imageEditMode !== 'marker' || !imageMarkerDrag) return;
    const point = markerImagePointFromEvent(event);
    if(!point) return;
    event.preventDefault();
    event.stopPropagation();
    const marker = currentMarkerImage().markers.find(item => item.id === imageMarkerDrag.markerId);
    if(!marker) return;
    marker.x = point.x;
    marker.y = point.y;
    renderImageMarkers();
}
function endImageMarkerPointer(event){
...
function isControllerPanelSafeTarget(target){
    ...
    if(target.closest?.('.controller-open-panel')) return true;
    return false;
}
...
function handleControllerPanelOutsidePointer(event){
...
document.addEventListener('pointerdown', handleControllerPanelOutsidePointer, true);
...
function controllerSliderHtml(label, key, min, max, step, value, suffix='', extraClass=''){
    ...
    return `<div class="ctrl-slider ${extraClass}" style="--slider-fill:${(fill*100).toFixed(2)}%;" data-slider="${key}" data-min="${min}" data-max="${max}" data-step="${step}" data-value="${v}" tabindex="0">
...
function syncSliderValue(slider){
    ...
    slider.style.setProperty('--slider-fill', (fill*100).toFixed(2) + '%');
...
function bindSliderPrimitive(slider, node, applyValue){
    ...
    slider.addEventListener('wheel', e => {
        ...
        const dir = controllerWheelDirection(e);
...
const CONTROLLER_WHEEL_CYCLE_ZONES = new Set([
    'body', 'category', 'focal', 'vibe', 'shot', 'aperture', 'motion',
    'angle-preset', 'subject-group',
    'light-type', 'light-direction'
]);
function controllerWheelDirection(event){
    return Number(event?.deltaY || 0) > 0 ? 1 : -1;
}
...
function bindControllerPanel(node){
    ...
    controllerPanel.onwheel = e => {
        ...
        const matDetail = e.target.closest && e.target.closest('.material-detail, .material-detail-form');
        if(matDetail){
            const scroller = matDetail.closest('.controller-panel-content');
            if(!scroller) return;
        ...
        if(lens){
            e.preventDefault(); e.stopPropagation();
            cycleChipRow(lens, controllerWheelDirection(e), () => CAMERA_PRESETS.aperture, () => state.camera.aperture, p => updateController(node, s => { s.camera.aperture = p.id; }, {hardRefresh:true}));
            return;
        }
        const zone = e.target.closest && e.target.closest('[data-zone]');
        if(zone){
            const z = zone.dataset.zone;
            if(!CONTROLLER_WHEEL_CYCLE_ZONES.has(z)){
...
function defaultNodeSize(type){
    ...
    if(type === 'controller') return {w:300, h:0};
...
function renderGeneratorBody(node){
    ...
    const syncSizeControls = () => {
        ...
        if(ratioSelect.value !== node.ratio && [...ratioSelect.options].some(opt => opt.value === node.ratio)) ratioSelect.value = node.ratio;
        if(resolutionSelect.value !== (node.resolution || defaultApiImageResolution(node.model))) resolutionSelect.value = node.resolution || defaultApiImageResolution(node.model);
        syncQualityControls();
...
function promptRichHtmlFromText(text='', promptNode=null){
    ...
    const escaped = [...new Set(markerTexts)].sort((a, b) => b.length - a.length).map(s => s.replace(/[.*+?^${}()|[\]\\]/g, '\\function renderImageInputList(list, node, imageInputs, emptyText=null){'));
...
async function runRhModelNode(node, opts={}){
    ...
    try {
        ...
        if(!out && err?.jimengPending && handoffCanvasNoOutputJimengTask(node, run, err, {
            refs,
...
function mediaRefsFromNode(node){
    ...
    if(node.type === 'group'){
        return (node.items || [])
            ...
            .map(item => imageRefWithMarkers(item, {kind:mediaKindForNode(item)}));
    }
    if(node.type === 'output'){
...
async function runGenerator(genId, opts={}){
    ...
    try {
        ...
        if(!out && err?.jimengPending && handoffCanvasNoOutputJimengTask(gen, run, err, {
            refs,
            requestSize:payload.size,
            cascadeTargetId,
            providerId:payload.provider_id,
...
async function chooseAndOpenExternalApp(app, options={}){
    ...
    try {
        ...
        const pickRes = await fetch('/api/native/choose-executable', {
            method:'POST',
            headers:{'Content-Type':'application/json'},
            body:JSON.stringify({app, force:!!options.forceChoose})
        });
...
function openOutputExternalMenu(clientX, clientY){
    ...
    outputExternalMenu.style.left = `${clientX}px`;
    outputExternalMenu.style.top = `${clientY}px`;
...
function canvasJimengPendingError(taskData={}, canvasTaskId=''){
    const submitId = String(taskData?.submit_id || '');
    ...
    error.canvasTaskId = canvasTaskId;
...
function handoffCanvasNoOutputJimengTask(node, run, error, options={}){
    ...
    const out = {
        id:uid('out'),
        type:'output',
        x:Number(node.x || 0) + Number(options.dx || 460),
        y:Number(node.y || 0),
        images:[],
        _pending:[]
    };
...
async function waitCanvasImageTaskResult(taskId, options={}){
    ...
    while(true){
        ...
        const data = await res.json();
        if(data.status === 'succeeded') return data.result || {};
        if(data.status === 'jimeng_pending') throw canvasJimengPendingError(data, taskId);
...
function openOutputLightbox(url, out){
    ...
    if(outputDownloadAllBtn){
        outputDownloadAllBtn.style.display = (groupDownloadItems.length > 1 || outputDownloadItems.length > 1) ? 'flex' : 'none';
    ...
    if(videoMode){
        ...
        outputDownloadBtn.onclick = async e => {
            e.stopPropagation();
            try {
...
function canConnect(fromId, toId){
    ...
    if(from.type === 'controller') return CANVAS_GENERATOR_TYPES.includes(to.type) || to.type === 'llm';
...
let canvasClickZoomGesture = {count:0, timer:null, lastX:0, lastY:0, time:0};
function resetCanvasClickZoomGesture(){
    if(canvasClickZoomGesture.timer) clearTimeout(canvasClickZoomGesture.timer);
    canvasClickZoomGesture = {count:0, timer:null, lastX:0, lastY:0, time:0};
}
