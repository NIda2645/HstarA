...
const API_PROTOCOLS = ['openai', 'apimart', 'gemini', 'linapi', 'otuapi', 'grsai', 'toapis', 'baofu', 'bananarouter', 'moonly', 'volcengine', 'runninghub', 'jimeng', 'codex', 'gemini-cli'];
const PROVIDER_PROTOCOL_OPTIONS = API_PROTOCOLS;
...
function apiResponseErrorMessage(data, text, fallback){
    const detail = data && typeof data === 'object'
        ? (data.detail || data.message || data.error_description || data.error)
        : '';
    if(detail){
        if(typeof detail === 'string') return detail;
        try { return JSON.stringify(detail).slice(0, 300); } catch(e) {}
    }
    return compactResponseText(text) || fallback || '请求失败';
}
async function readApiJsonResponse(response, fallback='请求失败'){
    ...
    if(text){
        try { data = JSON.parse(text); }
            ...
            if(response.ok){
                throw new Error(compactResponseText(text) || fallback || '响应不是有效 JSON');
            }
        }
    }
    if(!response.ok){
        throw new Error(apiResponseErrorMessage(data, text, fallback));
    }
    return data || {};
}
...
function isApimartProviderContext(item){
    ...
    return baseUrl.includes('apimart.ai') || baseUrl.includes('img.688.qzz.io');
}
...
function detectProviderProtocolFromUrl(baseUrl){
    ...
    if(url.includes('linapi.net')) return 'linapi';
    if(url.includes('otuapi.com')) return 'otuapi';
    if(url.includes('grsai.dakka.com.cn') || url.includes('grsaiapi.com')) return 'grsai';
    if(url.includes('toapis.com')) return 'toapis';
    if(url.includes('img.688.qzz.io')) return 'apimart';
    if(url.includes('baofu')) return 'baofu';
    if(url.includes('bananarouter.com')) return 'bananarouter';
    if(url.includes('relay.moonlyai.com')) return 'moonly';
    if(url.includes('volcengine') || url.includes('volces.com')) return 'volcengine';
    if(url.includes('runninghub.cn') || url.includes('runninghub.ai')) return 'runninghub';
    return '';
}
