(function bootstrapOpenShopTextTools(root){
  ...
  function createController(options = {}){
      ...
      const runApply = async () => {
        try {
          ...
          blocks.forEach((block, index) => {
            ...
            const writingMode = normalizeWritingMode(block?.writingMode);
            const inferredSize = Math.max(1, (writingMode === 'vertical' ? geometry.width : geometry.height) * 0.8);
    ...
    function reviewHtml(){
      ...
      const reviewApplied = Boolean(activeOcrReviewRecord()?.appliedAt);
      ...
      return `<section class="hstar-text-section"><div class="hstar-text-label">识别校对</div>
        ...
        <div class="hstar-text-actions"><button type="button" class="btn btn-primary" data-hstar-action="apply-extraction" ${applyPending || reviewApplied ? 'disabled' : ''}>${reviewApplied ? '已创建文字图层' : '确认并创建文字图层'}</button></div>
      </section>`;
    }
